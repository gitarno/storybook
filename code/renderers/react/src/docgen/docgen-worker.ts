 ...
 * preset.
 *
 * This provider takes no config: `react-component-meta` extraction reads nothing from the user's
 * `typescript` options. `ComponentMetaManager` builds its own program, and the only docgen-relevant
 * TS option (`reactDocgenTypescriptOptions`, including `propFilter`) applies solely to the
 * `react-docgen-typescript` engine used by the legacy manifest path — not here.
 */

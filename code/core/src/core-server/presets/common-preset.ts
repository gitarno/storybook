...
export const services = async (_value: void, options: Options): Promise<void> => {
  ...
  if (features?.experimentalDocgenServer && !options.ignorePreview) {
    ...
    // the dev-server event loop. The worker composes the descriptor chain; here we only hand the
    // service an `extract` that forwards one component to it. When the compiled worker script is
    // unavailable (e.g. running from source without a build) the client is undefined and we skip
    // docgen registration rather than extracting on the main thread.
    const docgenWorker =
      docgenDescriptors.length > 0 ? createDocgenWorkerClient(docgenDescriptors) : undefined;

    registerDocgenServices({
      getIndex: () => storyIndexGenerator.getIndex(),
      docgenProvider: docgenWorker ? (input) => docgenWorker.extract(input.entry) : undefined,
      storyDocsProvider,
      workingDir: process.cwd(),
      // Build the first component's TypeScript program up front (off the main thread) in dev so the
      // first Controls panel populates without waiting on a cold program build.
      warmFirstComponent: options.configType === 'DEVELOPMENT',
    });
  }
};

// Unit tests for the docgen worker client's dispatch / readiness / failure / dispose plumbing. A
// real worker is replaced with an EventEmitter stand-in so the tests stay hermetic.
...
vi.mock('../../../../utils/module.ts', () => ({
  importMetaResolve: vi.fn(() => 'file:///fake/storybook/docgen-worker.js'),
}));
...
describe('createDocgenWorkerClient', () => {
  ...
  it('spawns a worker and posts the init message with descriptors', async () => {
    const { createDocgenWorkerClient } = await loadModule();
    const client = createDocgenWorkerClient(DESCRIPTORS);

    expect(client).toBeDefined();
    expect(fakeWorkers).toHaveLength(1);
    expect(fakeWorkers[0].posted[0]).toEqual({ type: 'init', descriptors: DESCRIPTORS });
    expect(fakeWorkers[0].unref).toHaveBeenCalled();
...
describe('DocgenWorkerClient.extract', () => {
  ...
  it('rejects after dispose', async () => {
    ...
    await expect(client.extract({ id: 'x' } as any)).rejects.toThrow('docgen worker disposed');
  });
});

 ...
 * `moduleSpecifier` pointing at a module that satisfies {@link DocgenWorkerModule}, plus a
 * structured-clone-safe `config` forwarded to that module's factory inside the worker. Core
 * collects these descriptors (preserving preset order) and the worker imports and composes them.
 */
export interface DocgenProviderDescriptor {
  ...
  /** Structured-clone-safe data passed to the module's `createDocgenProvider` factory. */
  config?: unknown;
}

/**
 * Contract a worker-target docgen module must satisfy. The worker imports the descriptor's
 * `moduleSpecifier` and calls `createDocgenProvider(config)` once to build the middleware it folds
 * into the provider chain. Integrations implement only this factory — they never touch threading.
 */
export interface DocgenWorkerModule {
  createDocgenProvider: (config: unknown) => DocgenMiddleware | Promise<DocgenMiddleware>;
}
